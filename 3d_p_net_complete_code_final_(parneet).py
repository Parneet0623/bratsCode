# -*- coding: utf-8 -*-
"""3d_p_net_complete_code_Final_(Parneet).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1WG5GQ9QbV2A-6vBKJ-eHAccXsosz0pbV
...
sample_filename = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001_flair.nii.gz'
sample_filename_mask = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001_seg.nii.gz'
...
sample_filename2 = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001_t1.nii.gz'
sample_img2 = nib.load(sample_filename2)
sample_img2 = np.asanyarray(sample_img2.dataobj)
sample_img2  = np.rot90(sample_img2)

sample_filename3 = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001_t2.nii.gz'
sample_img3 = nib.load(sample_filename3)
sample_img3 = np.asanyarray(sample_img3.dataobj)
sample_img3  = np.rot90(sample_img3)

sample_filename4 = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001_t1ce.nii.gz'
...
class GlobalConfig:
    root_dir = '/content/sample_data/brats20-dataset-training-validation'
    train_root_dir = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData'
    test_root_dir = '/content/sample_data/brats20-dataset-training-validation/BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData'
...
survival_info_df = pd.read_csv('/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/survival_info.csv')
name_mapping_df = pd.read_csv('/content/sample_data/brats20-dataset-training-validation/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/name_mapping.csv')
...
class Basic3DBlock(nn.Module):
    def __init__(self, in_size, out_size, kernel_size, activation=F.relu, stride=1, padding=1, dilation=1):
        super(Basic3DBlock, self).__init__()
        #self.conv = nn.Conv2d(in_channels=in_size, out_channels=out_size, kernel_size=kernel_size, padding=padding)
        self.conv = nn.Conv3d(in_channels=in_size, out_channels=out_size, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation)
...
class PNet3D(nn.Module):
    def __init__(self, channel_count, class_count):
        super(PNet3D, self).__init__()
        #self.imsize = imsize
        self.conv_block1 = nn.Sequential(
            Basic3DBlock(channel_count, 64, 3, F.relu, 1, 1, 1),
            Basic3DBlock(64, 64, 3, F.relu, 1, 1, 1)
        )
        self.block1_encoder = Basic3DBlock(64, 16, 1, F.relu, 1, 0, 1)
        self.conv_block2 = nn.Sequential(
            Basic3DBlock(64, 64, 3, F.relu, 1, 2, 2),
            Basic3DBlock(64, 64, 3, F.relu, 1, 2, 2)
        )
        self.block2_encoder = Basic3DBlock(64, 16, 1, F.relu, 1, 0, 1)
        self.conv_block3 = nn.Sequential(
            Basic3DBlock(64, 64, 3, F.relu, 1, 3, 3),
            Basic3DBlock(64, 64, 3, F.relu, 1, 3, 3)
        )
        self.block3_encoder = Basic3DBlock(64, 16, 1, F.relu, 1, 0, 1)
        self.conv_block4 = nn.Sequential(
            Basic3DBlock(64, 64, 3, F.relu, 1, 4, 4),
            Basic3DBlock(64, 64, 3, F.relu, 1, 4, 4)
        )
        self.block4_encoder = Basic3DBlock(64, 16, 1, F.relu, 1, 0, 1)
        self.conv_block5 = nn.Sequential(
            Basic3DBlock(64, 64, 3, F.relu, 1, 5, 5),
            Basic3DBlock(64, 64, 3, F.relu, 1, 5, 5)
        )
        self.block5_encoder = Basic3DBlock(64, 16, 1, F.relu, 1, 0, 1)
        self.conv_block6 = nn.Sequential(
            Basic3DBlock(80, 3, 1, F.relu, 1, 0, 1),
            nn.Softmax(dim=1)
        )
    def forward(self, x):
        #print(x.size())
        ...
        #print(block5.size())
        block11 = self.block1_encoder(block1)
        #print(block11.size())
        block21 = self.block2_encoder(block2)
        #print(block21.size())
        block31 = self.block3_encoder(block3)
        #print(block31.size())
        block41 = self.block4_encoder(block4)
        # print(block41.size())
        block51 = self.block5_encoder(block5)
        #print(block51.size())
        concatenated_feature_map = torch.cat((block11, block21, block31, block41, block51), 1)
        concatenated_feature_map_drop = F.dropout(concatenated_feature_map, p=0.5, training=self.training)
        block6 = self.conv_block6(concatenated_feature_map_drop)
        return block6
...
nodel = PNet3D(channel_count=4, class_count=3).to('cuda')
